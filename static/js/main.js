document.addEventListener('DOMContentLoaded', function () {
    ...
    //             if (priceText) {
    //                 total += parseFloat(priceText.replace(/[^\d.,]/g, '').replace(',', '.'));
    //             }
    //         });
    //         if (cartTotalPriceElem) cartTotalPriceElem.textContent = `${total.toFixed(0)} ₽`;
    //     }
    //
    //     if (cartItemsList) {
    ...
    //             if (!action) return;
    //
    //             // Оптимистичное обновление DOM
    //             if (action === 'remove' || quantity === 0) {
    //                 cartItem.remove();
    //             } else {
    //                 quantityElem.textContent = quantity;
    //                 itemTotalElem.textContent = `${(basePrice * quantity).toFixed(0)} ₽`;
    //             }
    //             updateCartTotal();
    //
    //             // Отправка на сервер
    ...
    //             }).then(response => {
    //                 if (!response.ok) {
    //                     // Если сервер вернул ошибку — перезагружаем страницу
